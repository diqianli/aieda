...
| 文件 | 说明 |
|------|------|
| `cpu_emulator.exe` | 主程序 - 生成 Konata 可视化数据 |
| `run.bat` | 一键运行脚本 |
| `static/` | 可视化网页文件目录 |
...
```cmd
cpu_emulator.exe [指令数量] [输出路径]

示例：
cpu_emulator.exe 10000 output.json        # 1万条指令
cpu_emulator.exe 100000 output.json        # 10万条指令
cpu_emulator.exe 1000000 output.json       # 100万条指令（需要较长时间）
```
...
```cmd
cd 解压目录
cpu_emulator.exe 10000 test.json
```
...
A: 减少指令数量：
```cmd
cpu_emulator.exe 10000 output.json
```
